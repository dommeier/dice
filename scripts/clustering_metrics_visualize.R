...
p3_denoised <- DimPlot(data_atlas_denoised, group.by = cell_names, pt.size = 1, cols = col_vector[11:40]) + ggtitle("DICE Embeddings") + theme(plot.title = element_text(hjust = 0.5))
p3_true <- DimPlot(data_atlas_true, group.by = cell_names, pt.size = 1, cols = col_vector[11:40]) + ggtitle("PCA Embeddings") + theme(plot.title = element_text(hjust = 0.5))

# Save DimPlot outputs as PDF files in the results folder
# Individual PDFs
pdf(file = file.path(folder_path, "dimplot_denoised.pdf"), width = 10, height = 8)
print(p3_denoised)
dev.off()

pdf(file = file.path(folder_path, "dimplot_true.pdf"), width = 10, height = 8)
print(p3_true)
dev.off()

# Combined PDF (side-by-side)
pdf(file = file.path(folder_path, "dimplot_comparison.pdf"), width = 20, height = 8)
print(p3_denoised + p3_true)
dev.off()

cat("DimPlot PDFs written to:\n")
cat(" -", file.path(folder_path, "dimplot_denoised.pdf"), "\n")
cat(" -", file.path(folder_path, "dimplot_true.pdf"), "\n")
cat(" -", file.path(folder_path, "dimplot_comparison.pdf"), "\n")
...
# Save metrics to CSV
results <- data.frame(
  Dataset = rep(c("Denoised", "True"), each = 5),
  Metric = rep(c("Adjusted_Rand_Index", "V_Measure", "Mean_LISI_Score", "Mean_Silhouette_Score", "Silhouette_Benchmark"), 2),
  Value = c(
    metrics_denoised$adj_r_index, metrics_denoised$v_measure,
    metrics_denoised$lisi_score, metrics_denoised$silhouette_score, silhouette_benchmark_denoised,
    metrics_true$adj_r_index, metrics_true$v_measure,
